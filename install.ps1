...
#change the parameters with comments
param(
	[string]$EngineSuffix = 'sc9', # engine suffix
    [string]$SitePrefix = "yourprefix", # site prefix
	[string]$SiteName = "$SitePrefix.sc", # check your sitecore install folder for the correct sitename
	[string]$ScInstallDirectory = "C:\inetpub\wwwroot", # site install directory
	[string]$SiteHostHeaderName = "storefront.local", # site host header name you want to use
	[string]$SqlServer = "XXXXXX\SQL2016", # SQL SERVER
	[string]$CommerceEngineCertificatePath = "..\storefront.local.cer", #the certificate for Commerce engine
	[string]$CommerceSearchProvider = "SOLR", 
	[string]$SolrUrl = "https://localhost:8983/solr", # Solr URL
	[string]$SolrRoot = "C:\solr\solr-6.6.2", # solr install directory
	[string]$SolrService = "solr-6.6.2" #solr service name
)
...
$params = @{
		...
		SitecoreCoreDbName = "$($SitePrefix)_Core"
